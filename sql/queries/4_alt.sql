...
inner join trajectories_ll s4 on (s3.userid = s4.userid and s3.tpos < s4.tpos and extract(DAY FROM s3.utctimestamp) = extract(DAY FROM s4.utctimestamp)) 
...
inner join (
	...
	from(
		...
		inner join trajectories_ll s2 on (s1.userid = s2.userid and s1.tpos < s2.tpos and extract(DAY FROM s1.utctimestamp) = extract(DAY FROM s2.utctimestamp)) 
