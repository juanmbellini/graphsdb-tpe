...
inner join trajectories_ss s2 on (s1.userid = s2.userid and s1.venueid = s2.venueid and s1.tpos < s2.tpos and extract(DAY FROM s1.utctimestamp) = extract(DAY FROM s2.utctimestamp)) 
